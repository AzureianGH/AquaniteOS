...
ControlledStrings parse_page_fault_bits(uint64_t error_code) {
    //check bits of fault
    ControlledStrings strings;
    strings.strings = (char**)malloc(3 * sizeof(char*));
    strings.numberof = 0;
    if (error_code & 0x1) {
        strings.strings[strings.numberof] = (char*)"Present";
        strings.numberof++;
    }
    if (error_code & 0x2) {
        strings.strings[strings.numberof] = (char*)"Write";
        strings.numberof++;
    }
    if (error_code & 0x4) {
        strings.strings[strings.numberof] = (char*)"User";
        strings.numberof++;
    }
    if (error_code & 0x8) {
        strings.strings[strings.numberof] = (char*)"Reserved";
        strings.numberof++;
    }
    if (error_code & 0x10) {
        strings.strings[strings.numberof] = (char*)"Instruction fetch";
        strings.numberof++;
    }
    return strings;
}
...
extern "C" void ISRHandler(registers_t *r) {
    if (r->isrNumber < 32) {
        ...
        lprintf(logging_level::ERROR, "Error code: 0x%l\n", r->errorCode);
        //check if page fault
        if (r->isrNumber == 14) {
            ControlledStrings strings = parse_page_fault_bits(r->errorCode);
            for (uint64_t i = 0; i < strings.numberof; i++) {
                lprintf(logging_level::ERROR, "Page fault: %s\n", strings.strings[i]);
            }
        }
